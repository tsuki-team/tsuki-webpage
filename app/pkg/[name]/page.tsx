...
function VersionItem({
  version, channel, active, onClick,
}: {
  version: string; channel: Channel; active: boolean; onClick: () => void;
}) {
  return (
    <button
      onClick={onClick}
...
function MetaRow({ label, value }: { label: string; value: string }) {
  return (
    <div style={{
      ...
      <span style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--fg-faint)", letterSpacing: "0.04em", textTransform: "uppercase" }}>
        {label}
      </span>
...
interface PkgDetail {
  readme: string | null;
  toml:   null; // we don't use toml detail in this view yet
}
...
export default function PkgDetailPage() {
  ...
  // Load package metadata list to find this package
  useEffect(() => {
    if (!name) return;
    fetch("/api/pkg/list")
      .then(r => r.json())
  ...
  // Load readme + toml when pkg/version/channel change
  const loadDetail = useCallback((pkgName: string, channel: Channel, version: string) => {
    setDetail(null);
    fetch(`/api/pkg/${pkgName}?channel=${channel}&version=${version}`)
      .then(r => r.json())
      .then((d: PkgDetail) => { setDetail(d); setLoading(false); })
      .catch(() => setLoading(false));
  }, []);

  useEffect(() => {
    if (!pkg || !selVer) return;
    loadDetail(pkg.name, selCh, selVer);
  }, [pkg, selCh, selVer, loadDetail]);

  // Build sorted version list across channels
  const versionList: { version: string; channel: Channel }[] = pkg
    ? Object.keys(pkg.versions)
        .sort((a, b) => b.localeCompare(a, undefined, { numeric: true }))
        .map(v => ({
          version: v,
          channel: pkg.channels.includes("stable") ? "stable" : "testing",
        }))
    : [];
      ...
      <>
        ...
        <main style={{ paddingTop: 52, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ textAlign: "center" }}>
            <p style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--fg-faint)", letterSpacing: "0.08em", textTransform: "uppercase" }}>
              Package not found
            </p>
            <a href="/pkg" style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--fg-muted)", textDecoration: "underline", textUnderlineOffset: 2 }}>
              ← Back to registry
            </a>
          </div>
        </main>
      ...
      <main style={{ paddingTop: 52, minHeight: "100vh", background: "var(--surface)" }}>
          ...
          <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 28px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <h1 style={{
                  fontFamily: "var(--font-mono)", fontSize: "clamp(20px, 3vw, 28px)",
                  fontWeight: 600, letterSpacing: "-0.02em", color: "var(--fg)", margin: 0,
                }}>
                  {pkg?.name ?? name}
                </h1>
              </div>
            ...
            {pkg && (
              <p style={{
                fontFamily: "var(--font-sans)", fontSize: 13.5, color: "var(--fg-muted)",
                lineHeight: 1.6, margin: "10px 0 0", maxWidth: 600,
              }}>
                {pkg.description}
              </p>
            )}
        ...
        <div style={{
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6, position: "sticky", top: 72 }}>
            ...
            {/* Install */}
            {pkg && selVer && (
              <InstallBox pkgName={pkg.name} version={selVer} channel={selCh} />
            )}
          ...
          <div style={{ minWidth: 0 }}>
            ...
            {loading ? (
              ...
              // API reference — shows functions from the TOML
...
function ApiReference({ pkgName, channel, version }: { pkgName: string; channel: Channel; version: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/pkg/${pkgName}?channel=${channel}&version=${version}`)
      .then(r => r.json())
      .then((d: { toml: import("@/lib/pkg-fetcher").ParsedToml | null }) => {
        setToml(d.toml);
        setLoading(false);
      })
      .catch(() => setLoading(false));
  ...
  if (loading) return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      {[60, 80, 50].map((w, i) => (
        <div key={i} style={{ height: 13, width: `${w}%`, background: "var(--surface-2)", borderRadius: 3, animation: "pulse 1.5s ease-in-out infinite" }} />
      ))}
    </div>
  );

  if (!toml) return (
    <p style={{ fontFamily: "var(--font-mono)", fontSize: 10.5, color: "var(--fg-faint)", textTransform: "uppercase", letterSpacing: "0.06em" }}>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {(toml.cpp_header || toml.arduino_lib) && (
        <div style={{ background: "var(--surface-2)", border: "1px solid var(--border)", borderRadius: 6, padding: "12px 14px", display: "flex", flexDirection: "column", gap: 6 }}>
          {toml.cpp_header  && <MetaRowInline label="C++ Header"    value={toml.cpp_header} />}
          {toml.arduino_lib && <MetaRowInline label="Arduino Lib"   value={toml.arduino_lib} />}
          {toml.cpp_class   && <MetaRowInline label="C++ Class"     value={toml.cpp_class} />}
          {toml.aliases     && <MetaRowInline label="Aliases"       value={toml.aliases.join(", ")} />}
        </div>
      )}
      ...
      {toml.functions.length > 0 && (
        <div>
          <div style={{ fontFamily: "var(--font-mono)", fontSize: 9, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--fg-faint)", marginBottom: 10 }}>
            Functions ({toml.functions.length})
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {toml.functions.map((fn, i) => (
              <FnRow key={i} fn={fn} />
            ))}
          </div>
        </div>
      ...
      {toml.constants.length > 0 && (
        <div>
          <div style={{ fontFamily: "var(--font-mono)", fontSize: 9, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--fg-faint)", marginBottom: 10 }}>
            Constants ({toml.constants.length})
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {toml.constants.map((c, i) => (
              <FnRow key={i} fn={c} />
            ))}
          </div>
        </div>
...
function MetaRowInline({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      <span style={{ fontFamily: "var(--font-mono)", fontSize: 9.5, color: "var(--fg-faint)", letterSpacing: "0.06em", textTransform: "uppercase", flexShrink: 0, minWidth: 80 }}>
        {label}
      </span>
...
function FnRow({ fn }: { fn: { go?: string; python?: string; cpp?: string } }) {
  return (
    <div style={{
      ...
      {[
        { lang: "go",     val: fn.go,     color: "#6ba4e0" },
        { lang: "python", val: fn.python, color: "#3572a5" },
        { lang: "cpp",    val: fn.cpp,    color: "#e07898" },
      ].map(({ lang, val, color }) => (
        <div key={lang} style={{
          padding: "6px 10px",
          ...
          <span style={{ fontFamily: "var(--font-mono)", fontSize: 8.5, letterSpacing: "0.06em", textTransform: "uppercase", color }}>
            {lang}
          </span>
          <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: val ? "var(--fg-muted)" : "var(--fg-faint)", fontStyle: val ? "normal" : "italic" }}>
            {val ?? "—"}
          </span>
        </div>
      ))}
